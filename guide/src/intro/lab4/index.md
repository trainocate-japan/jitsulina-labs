...
#### 2. httpd (Apache HTTP Server) が、まだインストールされていないことを確認してください。確認方法は問いません
...
#### 3. httpd (Apache HTTP Server) を dnf コマンドでインストールしてください
...
#### 8. rpmコマンドを使って、ダウンロードしたrpmファイルをインストールしてください。結果はどうでしょうか？
  ...
  <br>
  Failed dependencies（依存性の欠如）などのエラーが発生し、インストールできない
  :::
