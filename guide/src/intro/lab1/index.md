...
### 演習：ディレクトリの確認、作成、移動
#### （事前確認） 以下のコマンドを実行し、現在ログインしているユーザーを確認してください
```
whoami
```
#### （事前確認）もし ec2-user でない場合は、以下のコマンドを実行して ec2-user に切り替えてください。パスワードを求められたら、`password` を入力してEnterキーを押下してください（入力したパスワードは表示されません＿
```
ec2-user
```
...
#### 2. うまく作成できないディレクトリがあったと思います。その原因を考え、エラーの起きない方法で再度実行してください
  ...
  ::: details 解答例
  dir2の下のdir21は、親ディレクトリであるdir2が作成されてからでないと作成できない。dir2を先に作成するか、mkdirの-pオプションを利用する。dir3も同様
  <br>
  `mkdir dir2/dir21`
  <br>
...
::: tip
vimコマンドを入力するときに日本語入力を解除するのを忘れないようにしてください
:::

::: tip
環境によっては、入力中の日本語が画面に反映されないかもしれません。確定すると反映されます。入力内容が間違っていても演習には影響ありませんので、間違いは気にせずに進めてください
:::
