...
#### 3. 以降の作業はroot権限が必要なものがあります。rootユーザーで行ってもsudoコマンドを利用してもどちらでも構いません

  ::: tip
  sudoでコマンドを実行する場合、今のままではうまくいかないと思います。原因を考えてみましょう。以降の作業はrootユーザーで行った方が楽ですが、実際の業務ではrootでの作業は禁止されている場合もあることを覚えておきましょう
  :::
...
#### 11. rootユーザーの権限での作業はここで終了です。rootユーザーで作業していた場合は、以下のコマンドを実行してec2-userのシェルに戻ってください。sudo でコマンドを実行していた場合は、そのまま次に進んでもかまいません
...
#### 13. idコマンドでユーザーの情報を表示し、うまく行かなかった原因を考えてから解答例を見てください
  ...
  ::: details 解答例
  ec2-userの所属するグループにcommonが含まれていないため、権限がなく失敗している
  <br>
