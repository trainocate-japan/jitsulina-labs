...
#### 9. これまでの作業結果を確認してください。/opt/sharedディレクトリのグループがcommonで、権限がrwxになっているはずです。確認の方法は問いません
  ...
  `ls -ld /opt/shared`
  <br>
  結果の例 `drwxrwxr-x. 2 root common 6 Apl 24 10:32 /opt/shared/`
  ::: 
...
#### 14. 上記の対策をとってから、再度/opt/sharedディレクトリにファイルを作成してみてください。今度はうまくいくはずです。前の手順でエラーが出なかった方は、この手順は飛ばしても構いません
  ...
  ::: details 解答例
  再度、`su - ec2-user`する、など。GUI環境の場合は`exit`やマウス操作などでターミナルウィンドウを閉じたあとに、再度ターミナルから ec2-user に入り直してもよいです
  <br>
  ファイルの作成方法は、`touch /opt/shared/hoge`、など何でも構いません
  ::: 
